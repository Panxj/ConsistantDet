        ...
        if self.with_rpn:
            if hasattr(self.neck, 'with_sfa'):
                if self.neck.with_orig:
                    ...
                    rpn_loss_inputs_orig = rpn_outs_orig + (gt_bboxes, img_meta,
                                                  self.train_cfg.rpn)
                ...
                if hasattr(self.neck, 'with_sfa'):
                    ...
                    if self.neck.with_sfa:
                        assign_result_sfa = bbox_assigner.assign(
                            proposal_list_sfa[i], gt_bboxes_up[i], gt_bboxes_ignore[i],
                            gt_labels[i])
        ...
        if self.with_bbox:
            if hasattr(self.neck, 'with_sfa'):
                ...
                if self.neck.with_sfa:
                    ...
                    bbox_targets = self.bbox_head.get_target(
                        sampling_results_sfa, gt_bboxes_up, gt_labels, self.train_cfg.rcnn)
                    loss_bbox = self.bbox_head.loss(cls_score, bbox_pred,
        ...
        if self.with_mask:
            if hasattr(self.neck, 'with_sfa'):
                ...
                if self.neck.with_sfa:
                    ...
                    mask_targets = self.mask_head.get_target(
                        sampling_results_sfa, gt_masks_up, self.train_cfg.rcnn)
                    pos_labels = torch.cat(
        ...
        if hasattr(self.neck, 'with_sfa'):
            if self.neck.with_sfa and self.neck.only_sfa_result:
                proposal_list_sfa = self.simple_test_rpn(
                    x[0], img_meta_up, self.test_cfg.rpn) if proposals is None else proposals
                det_bboxes_sfa, det_labels_sfa = self.simple_test_bboxes(
                    x[0], img_meta_up, proposal_list_sfa, self.test_cfg.rcnn, rescale=rescale)
                ...
                if not self.with_mask:
                    ...
                    segm_results_sfa = self.simple_test_mask(
                        x[0], img_meta_up, det_bboxes_sfa, det_labels_sfa, rescale=rescale)
                ...
                if not self.with_mask:
                    ...
                    segm_results_orig = self.simple_test_mask(
                        x[-1], img_meta, det_bboxes_orig, det_labels_orig, rescale=rescale,
                ...
                det_bboxes, det_labels= self.simple_test_bboxes_for_sfa_with_orig(
                    x[0], x[-1], img_meta_up, img_meta, proposal_list_sfa, proposal_list_orig,
                ...
                if not self.with_mask:
                    ...
                    segm_results = self.simple_test_mask_for_sfa_with_orig(
                        x[0], x[-1], img_meta_up, img_meta, det_bboxes, det_labels, rescale=rescale,
