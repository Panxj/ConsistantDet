...
from .py_utils.utils import convolution, residual, corner_pool ,DcnPool, feature_scatter
...
class model(hg_net):
    ...
    def __init__(self):
        ...
        tl_modules = nn.ModuleList([DcnPool(256, 128, 0, 1, (3, 3), padding=1, outmode=1, filter=True) for _ in range(stacks)])
        br_modules = nn.ModuleList([DcnPool(256, 128, 2, 3, (3, 3), padding=1, outmode=1, filter=True) for _ in range(stacks)])

        feasca_modules = nn.ModuleList([feature_scatter(256, 256, 2, 1) for _ in range(stacks)])
        ...
        super(model, self).__init__(
            res, tl_modules, br_modules, tl_heats, br_heats,
            tl_tags, br_tags, tl_offs, br_offs, feasca_modules=feasca_modules
        )

        self.loss = CornerNet_Loss(pull_weight=1e-1, push_weight=1e-1, dcn_off_weight=0)
